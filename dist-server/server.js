...
function broadcast(sessionCode, message, wss, excludeWs) {
...
function getVoteCount(ticket, session) {
...
function calculateAverage(ticket) {
    const values = Object.values(ticket.votes).filter((v) => v !== null);
    if (values.length === 0)
        return 0;
    return Math.round((values.reduce((a, b) => a + b, 0) / values.length) * 10) / 10;
}
export function handleMessage(ws, message, wss) {
    switch (message.type) {
        case 'join': {
            ...
            ws.send(JSON.stringify({ type: 'sessionState', session, userId: user.id }));
            // Notify others
        ...
        case 'addTicket': {
            ...
            if (!user || user.role !== 'observer') {
                ws.send(JSON.stringify({ type: 'error', message: 'Only observers can add tickets' }));
                return;
            }
            const ticket = {
                id: uuidv4(),
        ...
        case 'selectTicket': {
            ...
            const user = session.users.find(u => u.id === clientInfo.userId);
            if (!user || user.role !== 'observer') {
                ws.send(JSON.stringify({ type: 'error', message: 'Only observers can select tickets' }));
                return;
            }
        ...
        case 'vote': {
            ...
            if (!user || user.role !== 'player') {
                ws.send(JSON.stringify({ type: 'error', message: 'Only players can vote' }));
                return;
            }
            if (!session.selectedTicketId) {
                ws.send(JSON.stringify({ type: 'error', message: 'No ticket selected' }));
                return;
            }
            if (!POINT_VALUES.includes(message.points)) {
                ws.send(JSON.stringify({ type: 'error', message: 'Invalid point value' }));
                return;
            }
        ...
        case 'revealVotes': {
            ...
            const user = session.users.find(u => u.id === clientInfo.userId);
            if (!user || user.role !== 'observer') {
                ws.send(JSON.stringify({ type: 'error', message: 'Only observers can reveal votes' }));
                return;
            }
            if (!session.selectedTicketId) {
                ws.send(JSON.stringify({ type: 'error', message: 'No ticket selected' }));
        ...
        case 'resetVotes': {
            ...
            const user = session.users.find(u => u.id === clientInfo.userId);
            if (!user || user.role !== 'observer') {
                ws.send(JSON.stringify({ type: 'error', message: 'Only observers can reset votes' }));
                return;
            }
            if (!session.selectedTicketId) {
                ws.send(JSON.stringify({ type: 'error', message: 'No ticket selected' }));
...
export function handleDisconnect(ws, wss) {
    const clientInfo = clientSessions.get(ws);
    if (clientInfo) {
        const session = sessions[clientInfo.sessionCode];
        if (session) {
